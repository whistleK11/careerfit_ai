...
- [ ] 4일차: RAG 기반 서비스 + React UI
    - ChromaDB와 Gemini를 연동하여 RAG 기반 응답을 구현하고 /analyze API를 RAG 응답(answer + sources)으로 교체 완료
    - React + Vite 프로젝트를 생성하고 fetch를 이용해 FastAPI /analyze API와 연동하여 결과 카드(ResultCard, SourceCard) 출력 구현 완료
    - LLM API 지침을 RAG 근거, 구조화 응답, 신뢰도(confidence) 기준까지 확장하고 RAG_EVALUATION.md 및 OPTIMIZATION_LOG.md 작성 완료
    - FastAPI와 React를 통합 테스트하여 입력 → API 호출 → RAG 응답 → 화면 출력까지 전체 서비스 동작 검증 완료
- [ ] 5일차: Docker + 포트폴리오 완성
